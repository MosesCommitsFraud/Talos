...
async def run_auto_sort(owner: str, skip_llm: bool = False, delete_throwaway: bool = True) -> str:
    ...
    try:
        ...
        if len(session_list) < 2:
            return (
                f"Cleaned {deleted_empty + deleted_throwaway} sessions. Too few remaining to sort."
            )
        ...
        if not folders:
            return (
                f"Cleaned {deleted_empty + deleted_throwaway} sessions. No folder groupings found."
            )
