...
class DataConfig(BaseSettings):
    ...
    # Base directory
    base_dir: Path = Field(
        default=Path(__file__).parent.parent, description="Base directory for the application"
    )
...
class AppConfig(BaseSettings):
    ...
    def set_data_paths(cls, v, info):
        ...
        max_upload_size = (
            v.get("max_upload_size", 10 * 1024 * 1024) if isinstance(v, dict) else 10 * 1024 * 1024
        )
        allowed_extensions = (
            v.get(
                "allowed_extensions",
