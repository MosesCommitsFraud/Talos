...
def _ollama_normalize_tool_messages(messages: List[Dict]) -> List[Dict]:
    """Adapt Talos' canonical OpenAI-style messages to native Ollama /api/chat.

    Talos carries assistant tool calls in the OpenAI shape, where
...
def _provider_headers(provider: str, headers: Optional[Dict] = None) -> Dict[str, str]:
    ...
    if provider == "openrouter":
        h.setdefault("HTTP-Referer", "https://github.com/pewdiepie-archdaemon/talos")
        h.setdefault("X-OpenRouter-Title", "Talos")
    if provider == "copilot":
...
def _sanitize_llm_messages(messages: List[Dict]) -> List[Dict]:
    """Strip Talos-only metadata before sending messages to providers.
    ...
    while i < len(cleaned):
        ...
        pruned_calls = [
            tc for tc in tool_calls if isinstance(tc, dict) and str(tc.get("id")) in answered
        ]
        fixed = dict(msg)
...
def list_model_ids(
    base_chat_url: str, timeout: int = LLMConfig.DEFAULT_TIMEOUT, headers: Optional[Dict] = None
) -> List[str]:
...
async def llm_call_async(
    ...
    if _is_host_dead(target_url):
        raise HTTPException(
            503, f"Upstream {_host_key(target_url)} marked unreachable (cooldown active)"
        )
                                    ...
                                    if isinstance(delta, dict):
                                        ...
                                        for tc in delta.get("tool_calls") or []:
                                            ...
                                            if "arguments" in func:
                                                ...
                                                # Stream tool arg deltas for doc tools
                                                if func["arguments"] and _tc_acc[idx].get(
                                                    "name"
                                                ) in (
                                                    "create_document",
                                                    "update_document",
