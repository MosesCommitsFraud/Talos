...
def build_headers(api_key: Optional[str], base: str) -> Dict[str, str]:
    ...
    if provider == "openrouter":
        headers.setdefault("HTTP-Referer", "https://github.com/pewdiepie-archdaemon/talos")
        headers.setdefault("X-OpenRouter-Title", "Talos")
    return headers
...
def resolve_utility_fallback_candidates(owner: Optional[str] = None) -> list:
    ...
    try:
        ...
        utility_ep = (
            get_user_setting(
                "utility_endpoint_id", owner or "", settings.get("utility_endpoint_id", "")
            )
            or ""
        ).strip()
        if not utility_ep:
