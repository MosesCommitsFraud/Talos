...
- "Open/show <panel>" (documents, library, gallery, email, inbox, sessions, brain/memories, skills, settings, notes) → call `ui_control` with `open_panel <name>`. Panel aliases: library/doc/docs/document→documents, images→gallery, mail/inbox/emails→email, chats/history→sessions, memory/memories→brain, preferences→settings. CRITICAL: "open memory/memories/brain" / "open skills" / "open notes" / "open documents" means OPEN THE PANEL — call `ui_control`, NOT a manage/list tool. The "manage_*" tools list contents in chat; `ui_control open_panel` opens the visual modal the user is asking for.
...
<shell command>
```
Run any shell command. Output is returned to you. Use for: installing packages, checking files, git, curl, system info, etc. Everything runs INSIDE an isolated sandbox container — never on the host — and your working directory is your private workspace. Create files only within the workspace (relative paths); don't rely on anything written to `/tmp` or other absolute paths persisting or being visible to the user.
You're in an isolated sandbox — you may create and modify files freely here (bash redirects `>`/`>>`, heredocs, `tee`, `sed -i`, python that writes, etc.). PREFER `write_file` to create/rewrite a file and `edit_file` to change part of one: they render a clean before/after diff in the chat. Use bash redirects/heredocs when it's genuinely more convenient — generating many files at once, appending to logs, or capturing command output to a file. Everything you create lands in your workspace and shows up in the chat's files.
...
PLAN_MODE_DIRECTIVE = (
    ...
    "NOT done anything yet. Do NOT claim you created, wrote, ran, sent, or changed anything.\n\n"
    "ABSOLUTE RULE — DO NOT MUTATE ANYTHING. Every write/state-changing tool, "
    "including bash/python, is disabled this turn and will be rejected. Use only "
    "the read-only tools listed below to inspect files/search context.\n\n"
    "OUTPUT: present the plan as a GitHub-style checklist, one concrete step per line:\n"
    "- [ ] first action you will take once approved\n"
    "- [ ] next action\n"
    "Do not execute. End your turn with the checklist."
)
