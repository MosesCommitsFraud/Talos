                        ...
                        if jm:
                            ...
                            if urgency in ("critical", "high"):
                                try:
                                    ...
                                    # Deep-link to open the original email in Talos (if public URL is configured).
                                    ...
                                    # HTML alternative with a clickable "Open in Talos" button
                                    import html as _h
...
def _scheduled_poll_once() -> dict:
    ...
    try:
        ...
        kind_expr = "talos_kind" if "talos_kind" in cols else "'scheduled' AS talos_kind"
        ...
        for r in rows:
            ...
            try:
                ...
                talos_kind = r[10] if len(r) > 10 else "scheduled"
                ...
                outer["X-Talos-Origin"] = "talos-ui"
                outer["X-Talos-Kind"] = re.sub(r"[^A-Za-z0-9_.-]", "-", talos_kind or "scheduled")[:64]
                outer["X-Talos-Ref"] = sid
                if r[6]:
...
async def _scheduled_email_poller():
    ...
    also exposed via the `talos-mail poll-scheduled` CLI for
...
def _inprocess_pollers_enabled() -> bool:
    """Honour `TALOS_INPROCESS_POLLERS` — set to `0`/`false`/`no`/`off`
    to disable the asyncio tasks so a cron / systemd-timer setup driving
    `talos-mail poll-scheduled` is the sole external driver. The legacy
    ...
    import os
    raw = os.environ.get("TALOS_INPROCESS_POLLERS", "1").strip().lower()
...
def _start_poller():
    ...
    Skipped entirely when `TALOS_INPROCESS_POLLERS=0` — use that when
    ...
    if not _inprocess_pollers_enabled():
        logger.info(
            "In-process email pollers disabled (TALOS_INPROCESS_POLLERS=0); "
            "drive `talos-mail poll-scheduled` externally."
        )
        return
    import asyncio
