...
def extract_preset(chat_handler, preset_id) -> PresetInfo:
    ...
    temperature, max_tokens, system_prompt, char_name = chat_handler.validate_and_extract_preset(
        preset_id
    )
    return PresetInfo(
...
async def build_chat_context(
    ...
    _ctx_msg = (
        preprocessed.enhanced_message if use_enhanced_message else preprocessed.text_for_context
    )
    ...
    if rag_sources:
        ...
        if rag_msgs:
            ...
            if target_idx is not None and rag_text:
                ...
                logger.info(
                    "Merged RAG context into current user turn (%d sources)", len(rag_sources)
                )
            else:
...
def public_rag_sources(sources: list) -> list:
    ...
    return [
        {k: v for k, v in (s or {}).items() if not str(k).startswith("_")} for s in (sources or [])
    ]
...
def _extract_thinking_meta(text: str) -> dict | None:
    ...
    think_match = re.match(
        r"^[\s]*<think(?:ing)?>([\s\S]*?)</think(?:ing)?>\s*([\s\S]*)", clean, re.IGNORECASE
    )
    if think_match:
