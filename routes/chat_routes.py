...
from core.database import ModelEndpoint, SessionLocal, set_session_mode
...
def setup_chat_routes(
    ...
    async def chat_endpoint(request: Request, chat_request: ChatRequest) -> Dict[str, str]:
        ...
        if _clear_orphaned_session_endpoint(sess, owner=owner):
            raise HTTPException(
                400, "Selected model endpoint was removed. Pick another model in Settings."
            )
        ...
        if needs_auto_name(sess.name):
            asyncio.create_task(auto_name_session(session_manager, sess, lang=chat_request.lang))
    ...
    async def chat_stream(request: Request) -> StreamingResponse:
        ...
        try:
            ...
            _has_atts = bool(
                body and isinstance(body.get("attachments"), list) and body["attachments"]
        ...
        async def stream_with_save() -> AsyncGenerator[str, None]:
            ...
            if _is_image_generation_session(sess, owner=_user):
                ...
                try:
                    ...
                    try:
                        _max_rounds = int(
                            get_setting("agent_max_rounds", _DEFAULT_ROUNDS) or _DEFAULT_ROUNDS
                        )
                    ...
                    async for chunk in stream_agent_loop(
                        ...
                        if chunk.startswith("data: ") and not chunk.startswith("data: [DONE]"):
                            ...
                            if (
                                full_response
                                or thinking_response
                                or (last_metrics and last_metrics.get("tool_events"))
                            ):
                    ...
                    try:
                        ...
                        logger.exception(
                            "Failed to save partial response on disconnect (session %s)", session
                        )
                    raise
                finally:
                    _active_streams.pop(session, None)
