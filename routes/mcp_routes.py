...
def _mcp_oauth_base_dir() -> Path:
    """Directory that may contain OAuth files managed by Talos."""
        ...
        if resolve_pending(state, code):
            return HTMLResponse(
                _oauth_result_page(
                    ...
                    "The MCP server is connecting. You can close this window and return to Talos.",
                    success=True,
                )
            )
    ...
    async def _exchange_and_connect(server_id: str, code: str, request: Request):
        ...
        try:
            ...
            if resp.status_code != 200:
                ...
                return HTMLResponse(
                    _oauth_result_page("Authorization Failed", f"Google returned an error: {err}"),
                    status_code=400,
                )
...
<html><head>
<meta charset="UTF-8"><title>Authorize — Talos</title>
<style>
