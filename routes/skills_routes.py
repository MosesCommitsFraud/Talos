...
async def _eval_skill_run(
    skill_md: str, task: str, transcript: str, url: str, model: str, headers: Optional[dict]
) -> dict:
    ...
    def _parse(raw: str):
        ...
        if v not in _VERDICTS:
            km = _re.search(
                r'verdict["\'\s:]*\s*["\']?(pass|needs_work|fail|inconclusive)', text, _re.I
            )
            if km:
                v = km.group(1).lower()
                if data is None:
...
def _audit_finalize_status(
    ...
    if generic_reason:
        ...
        try:
            ...
            pass
    duplicate_of = (
        _skill_duplicate_blocker(skills_manager, name, owner) if verdict == "pass" else None
    )
    if duplicate_of:
        necessary = False
...
async def _run_audit_all_job(key, skills_manager, names, url, model, headers, teacher, owner):
    ...
    try:
        for nm in names:
            ...
            try:
                refreshed = next(
                    (s for s in skills_manager.load(owner=owner) if s.get("name") == nm), None
                )
                if refreshed:
                    res["skill_state"] = {
...
def _resolve_audit_models(owner=None):
    ...
    try:
        ...
        if _avail and model not in _avail:
            ...
            model = (
                next((a for a in _avail if _os.path.basename(a.rstrip("/")) == _base), None)
                or _avail[0]
            )
    except Exception:
        pass
...
def setup_skills_routes(skills_manager: SkillsManager) -> APIRouter:
    ...
    async def get_skill_markdown(request: Request, skill_id: str):
        ...
        skills = skills_manager.load(owner=user)
        match = next(
            (s for s in skills if s.get("name") == skill_id or s.get("id") == skill_id), None
        )
        if not match:
            raise HTTPException(404, "Skill not found")
    ...
    async def test_skill(request: Request, skill_id: str):
        ...
        try:
            ...
            if _avail and model not in _avail:
                ...
                _base = _os.path.basename((model or "").rstrip("/"))
                _match = next(
                    (a for a in _avail if _os.path.basename(a.rstrip("/")) == _base), None
                )
        ...
        _asyncio.create_task(
            _run_skill_test_job(key, name, md, task, url, model, headers, user, skills_manager)
        )
