...
def setup_session_routes(session_manager: SessionManager, config: dict, webhook_manager=None):
    ...
    def list_archived_sessions(request: Request):
        ...
        try:
            rows = (
                ...
                .filter(DbSession.archived == True)
                .all()
            )
        ...
        for s in user_sessions.values():
            ...
            last_msg = (
                (r.last_message_at.isoformat() if r and r.last_message_at else updated)
                if r
                else None
            )
            result.append(
    ...
    def create_session(
        ...
        if skip_val:
            ...
            ids = list_model_ids(endpoint_url, timeout=REQUEST_TIMEOUT, headers=validation_headers)
            if not ids:
    ...
    def auto_sort_sessions(request: Request, skip_llm: bool = False):
        ...
        try:
            for sid, folder_name in assignments.items():
                db_session = (
                    db.query(DbSession).filter(DbSession.id == sid, DbSession.owner == user).first()
                )
                if db_session:
