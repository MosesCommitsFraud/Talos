...
class LintTests(unittest.TestCase):
    ...
    def test_generated_mistakes_are_reported(self):
        css = (
            ".grid-card{background:#fff;box-shadow:0 1px 4px rgba(0,0,0,.06)}"
            ...
            td.echarts(
                {
                    ...
                    "series": [
                        {
                            "type": "bar",
                            "itemStyle": {"color": ["#4A90D9", "#E85D2A"]},
                            "label": {"formatter": "{c:,.0f}"},
                            "data": [1, 2],
                        }
                    ],
    ...
    def test_numbers_insights_and_overflow(self):
        layout = (
            "<p>207.5 Mio. € · 62.1% · 20748.5 K € · 33.500 € · Stand 31.12.2025</p>"
    ...
    def test_generated_wording_placeholders_layout_and_filters(self):
        layout = (
            '<div class="kpi">{{eur 236240409}}</div>'
        ...
        charts = [
            td.chart("a", "A", td.bar(["x"], [1])),
            td.chart("b", "B", td.echarts({"series": []})),
        ]
        issues = "\n".join(
            td.lint_composition(layout, ".g{grid-template-columns:repeat(12,1fr)}", charts)
        )
        for expected in (
            "em dash",
            "ganzheitlich",
    ...
    def test_logo_placeholder_inside_img_becomes_inline_svg(self):
        html = td.render(
            "Logo",
            [],
            layout_html='<img src="{{brand:logo}}" alt="Logo" class="brand-logo">',
            css="h1{}",
            brand="macs",
        )
