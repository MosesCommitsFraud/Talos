...
def chunks(text, **kwargs):
    docs = split_documents([Document(content=text, meta={"source": "manual.md", "owner": "alice"})], **kwargs)
...
def test_setext_and_nested_heading_parent_relationships():
    docs = chunks("Manual\n======\n\nIntro.\n\n## Setup\n\nInstall.\n\n### Linux\n\nCommands.")
    assert [d.meta["heading_path"] for d in docs] == [["Manual"], ["Manual", "Setup"], ["Manual", "Setup", "Linux"]]
...
def test_window_respects_authorization_and_document_version():
    ...
    result = expand_context(store, [hit(docs[1])], filters={"field": "meta.owner", "operator": "==", "value": "alice"})[0]
...
def test_md_productive_router_does_not_call_docling(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(rag, "_lane_docling", lambda _: (_ for _ in ()).throw(AssertionError("Docling called")))
...
def test_redaction_also_cleans_heading_and_original_docling_text():
    doc = Document(content="body", meta={"heading_path": ["bob@example.com"],
...
def test_section_continues_across_pages_but_page_provenance_stays_local():
    raw = [Document(content=text, meta={"source": "x.pdf", "page": page,
        "section_ordinal": 1, "section_path": [1], "heading_path": ["Chapter"]})
