...
def test_trailing_limit_becomes_top_on_mssql():
    q, note = _adapt_sql_dialect("SELECT * FROM dbo.ent273 WITH (NOLOCK) ORDER BY id273 LIMIT 50", "mssql")
...
def test_limit_left_alone_elsewhere_or_when_ambiguous():
    assert _adapt_sql_dialect("SELECT * FROM t LIMIT 5", "postgresql") == ("SELECT * FROM t LIMIT 5", "")
...
def test_error_hints_and_multi_statement_message():
    ...
    assert "DATENAME" in _sql_error_hint("'MONTHNAME' is not a recognized built-in function name", "mssql")
...
def test_file_tool_paths_are_mapped_to_the_workspace():
    ...
    assert _workspace_relative_path("output/build_dashboard.py", writing=True) == ("output/build_dashboard.py", None)
