...
import { artifactDownloadUrl, artifactsZipUrl, fetchArtifacts } from '@/api/client';
  ...
  return (
    <aside className="flex w-72 shrink-0 flex-col border-l bg-card" aria-label="Session files">
      ...
      <div className="min-h-0 flex-1 space-y-1 overflow-y-auto p-2">
        ...
        {sessionId && !isLoading && files.length === 0 && (
          <p className="px-2 py-6 text-center text-xs text-muted-foreground">
            No files yet. Files the agent writes in its workspace show up here.
          </p>
        )}
        {files.map((f) => {
