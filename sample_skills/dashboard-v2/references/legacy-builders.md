...
# Legacy TanStack builders (compatibility reference)

For new dashboards use the full ECharts API in ../SKILL.md. Restrictions in
this reference apply only to the existing builders, not to `td.echarts`.
...
The 28 types above are the legacy builder surface. For other types and full
ECharts options use `td.echarts`, described in ../SKILL.md. Existing builders
can share a dashboard with ECharts cards.
