// ============================================
// Talos UI — Main Application Orchestrator
...
async function _refreshDefaultChat() {
  try {
    ...
    if (d && d.endpoint_url && d.model) {
      _defaultChat = d;
      try { window.__talosDefaultChat = d; } catch (_) {}
      return d;
    }
  } catch (_) {}
  return null;
...
//  document.getElementById('chat-form').addEventListener('submit', chatModule.handleChatSubmit);
  ...
  if (exportPdfBtn) {
    exportPdfBtn.addEventListener('click', (e) => {
      ...
      const sessionName = meta ? meta.name : 'Talos Chat';
  ...
  document.addEventListener('keydown', (e) => {
    if (e.key === 'Escape') {
      ...
      const dynamicModals = ['library-modal', 'archive-modal', 'doclib-modal'];
  ...
  const _dynamicModalIds = ['library-modal', 'archive-modal', 'doclib-modal'];
  ...
  // URL-based panel routing — bookmark /memory, /cookbook etc
  ...
  // only displays the rail when `.sidebar.hidden` is set. Keeps the rail
  ...
  // Expose so other panels can call this without
  ...
  if (_opener) window._talosRouteOpener = _opener;
  ...
  if (modelSortBtn && modelSortDropdown) {
    ...
    modelSortDropdown.querySelectorAll('.sort-option').forEach(opt => {
      opt.addEventListener('click', () => {
        const mode = opt.dataset.sort;
        Storage.set('talos-model-sort', mode);
...
// Voice recording is handled by the dual-purpose send/mic button (see below)
  ...
  const SPLASH_COUNT_KEY = 'talos-tool-splash-counts';
  const SPLASH_MAX = 2;
  const _toolSplashes = {
  ...
  (function initModelPickerResponsive() {
    ...
    function checkPickerOverflow() {
      ...
      if (textarea) {
        textarea.setAttribute('placeholder', w < PLACEHOLDER_HIDE_WIDTH ? '' : 'Message Talos...');
      }
  ...
  const UI_VIS_KEY = 'talos-ui-visibility';
  ...
  (function migrateOldToolbarVis() {
    const OLD_KEY = 'talos-toolbar-visibility';
    try {
...
// ============================================
function startTalosApp() {
  if (window.__talosAppStarted) return;
  window.__talosAppStarted = true;
  // Set CSS variables
  ...
  if (documentModule) {
    ...
    if (_curSession && localStorage.getItem('talos-doc-open-' + _curSession) === '1') {
  ...
  if (sessionModule) {
    ...
    sessionModule.loadSessions()
      ...
      .finally(() => {
        ...
        if (window._talosRouteOpener) {
          try { window._talosRouteOpener(); } catch (_) {}
          window._talosRouteOpener = null;
        }
      });
  } else {
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', startTalosApp, { once: true });
} else {
  startTalosApp();
}
